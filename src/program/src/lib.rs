...
pub mod multivault {
    ...
    pub fn init_asset(
        ...
        require!(fee_bps <= 10_000, MultivaultErr::InvalidInput);
        ...
        require_keys_eq!(ctx.accounts.vault.owner, ctx.accounts.asset.key(), MultivaultErr::InvalidVault);
        require_keys_eq!(ctx.accounts.staking_vault.owner, ctx.accounts.asset.key(), MultivaultErr::InvalidVault);
        ...
        let mint_header = spl_if::Mint::try_deserialize(&mut shares_mint_ai.data.borrow().as_ref())
            .map_err(|_| MultivaultErr::InvalidMintAuthority)?;
        if let COption::Some(auth) = mint_header.mint_authority {
            require_keys_eq!(auth, ctx.accounts.asset.key(), MultivaultErr::InvalidMintAuthority);
        } else {
            // If mint_authority is None → immutable mint (not acceptable here)
            return err!(MultivaultErr::InvalidMintAuthority);
        }
    ...
    pub fn set_price_updater(ctx: Context<SetPriceUpdater>, updater: Pubkey) -> Result<()> {
        require_keys_eq!(ctx.accounts.asset.authority, ctx.accounts.authority.key(), MultivaultErr::Unauthorized);
    ...
    pub fn mint_shares(ctx: Context<MintShares>, amount: u64) -> Result<()> {
        require_keys_eq!(ctx.accounts.asset.authority, ctx.accounts.authority.key(), MultivaultErr::Unauthorized);
        require!(amount > 0, MultivaultErr::InvalidInput);
    ...
    pub fn stake(ctx: Context<Stake>, amount: u64) -> Result<()> {
        ...
        if pos.initialized == 0 {
            ...
            require_keys_eq!(pos.owner, ctx.accounts.user.key(), MultivaultErr::Unauthorized);
            require_keys_eq!(pos.asset, asset.key(), MultivaultErr::InvalidInput);
        }

        // accrue pending first
        let pending = pending_rewards(asset, pos)?;
        pos.accrued_rewards = pos.accrued_rewards.checked_add(pending).ok_or(MultivaultErr::Overflow)?;
        ...
        pos.shares_staked = pos.shares_staked.checked_add(amount).ok_or(MultivaultErr::Overflow)?;
        asset.total_staked = asset.total_staked.checked_add(amount).ok_or(MultivaultErr::Overflow)?;
    ...
    pub fn unstake(ctx: Context<Unstake>, amount: u64) -> Result<()> {
        ...
        let asset = &mut ctx.accounts.asset;
        let pos = &mut ctx.accounts.user_position;
        require!(pos.shares_staked >= amount, MultivaultErr::InsufficientStake);
    ...
    pub fn rent(ctx: Context<Rent>, days: u16) -> Result<()> {
        require!(days > 0, MultivaultErr::InvalidInput);
        let asset = &mut ctx.accounts.asset;
    ...
    pub fn claim(ctx: Context<Claim>) -> Result<()> {
        ...
        let amount = (pos.accrued_rewards as u128)
            ...
            .ok_or(MultivaultErr::Overflow)? as u64;
        require!(amount > 0, MultivaultErr::NothingToClaim);
    ...
    pub fn update_price(ctx: Context<UpdatePrice>, new_daily_price_usd: u64) -> Result<()> {
        ...
        require!(caller == ctx.accounts.asset.authority || caller == ctx.accounts.asset.price_updater, MultivaultErr::Unauthorized);
        require!(new_daily_price_usd > 0, MultivaultErr::InvalidInput);
    ...
    pub fn update_price_with_pyth(ctx: Context<UpdatePriceWithPyth>, ml_daily_price_usd: u64, max_age_sec: u64) -> Result<()> {
        let caller = ctx.accounts.caller.key();
        require!(caller == ctx.accounts.asset.authority || caller == ctx.accounts.asset.price_updater, MultivaultErr::Unauthorized);
        require!(ml_daily_price_usd > 0, MultivaultErr::InvalidInput);

        let clock = Clock::get()?;
        let feed = pyth_sdk_solana::load_price_feed_from_account_info(&ctx.accounts.pyth_price_feed)?;
        let price = feed.get_price_no_older_than(clock.slot, max_age_sec).ok_or(MultivaultErr::InvalidInput)?;
...
fn scale_i64_to_u64(v: i64, pow10: i32) -> Result<u64> {
    if pow10 >= 0 {
        ...
        let x = (v as i128).checked_mul(f as i128).ok_or(MultivaultErr::Overflow)?;
        require!(x >= 0, MultivaultErr::InvalidInput);
        Ok(x as u64)
    } else {
